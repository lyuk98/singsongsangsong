...
public class SongPlayListServiceImpl implements SongPlayListService{
	...
	@Override
	public SearchResponseDto searchArtistAndSong(String keyword, String requestGenre, String requestAtmosphere, Integer bpm,
		String sort) {
		int[] bpmRange = bpmRange(bpm);
		List<OrderSpecifier> orderSpecifiers = sortStandard(sort);
		List<Song> songListFiltering = songRepository.findSongByBpmAndKeyword(keyword, bpmRange[0],bpmRange[1],orderSpecifiers);
